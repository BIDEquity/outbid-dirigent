...
- **Apply BID Equity design system (web archetypes)**: Every greenfield web archetype MUST fetch and apply the BID Equity design system zip during scaffolding (`https://bid-equity-design-system.s3.eu-central-1.amazonaws.com/design-system.zip` → cache to `~/.cache/dirigent/design-system.zip`, extract, follow the embedded `SKILL.md`). The greenfield-scaffold skill enforces this as Step 4c — see `harness-docs/design-system.md` for the distribution + integration policy. Skip only when (a) non-web archetype, (b) SPEC explicitly forbids custom branding, or (c) target repo is a portfolio company prototype rather than a BID-owned product.
