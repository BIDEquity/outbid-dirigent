...
2. `${DIRIGENT_RUN_DIR}/SPEC.compact.json` — compacted spec with requirement IDs (if SPEC.md is missing, extract the requirements from this JSON and use them as the spec)
...
**If a valid `PLAN.json` already exists in `${DIRIGENT_RUN_DIR}`:** read it and validate it. If it has phases and tasks, return it as-is — do not regenerate.
