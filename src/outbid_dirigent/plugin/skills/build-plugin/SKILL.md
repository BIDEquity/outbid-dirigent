---
name: build-plugin
description: Scan a codebase and build a Claude Code plugin (.claude/) tailored to its stack, test setup, and dev workflow. Creates CLAUDE.md, skills, and agents specific to this project.
disable-model-invocation: true
---

# Build Plugin

Scan the current codebase and produce a complete `.claude/` configuration that makes Claude Code maximally effective for THIS specific project.
...
- Read `.claude/CLAUDE.md` if it exists — preserve user customizations
- Read `.claude/skills/` if it exists — don't duplicate existing skills
...
Use the plugin-writer agent to create the `.claude/` configuration.

Prompt for the agent:

> Build a `.claude/` configuration for this project based on the operational map below.
>
...
>
> Create:
> 1. `.claude/CLAUDE.md` — project instructions (under 100 lines)
> 2. `.claude/skills/run-tests/SKILL.md` — how to run tests in THIS project
> 3. `.claude/skills/dev-server/SKILL.md` — how to start dev environment (if applicable)
> 4. `.claude/skills/e2e-verify/SKILL.md` — how to verify features e2e (if applicable)
> 5. `.claude/agents/test-writer.md` — test specialist for THIS project's patterns (if useful)
>
...
- Read `.claude/CLAUDE.md` and confirm it has the right stack info
- List `.claude/skills/` and confirm skills were created
- Run one of the test commands from the skills to confirm it works

## Step 4: Report

Tell the user what was created:
- List every file in `.claude/` with a one-line description
...
- Do NOT overwrite existing `.claude/CLAUDE.md` without preserving user content (merge, don't replace)
