...
description: Greenfield-only variant of infra-architect — same brain, runs on Opus 4.7. Use when the SPEC starts from zero (no existing code) and the architectural decisions made here will constrain every downstream phase. The cost premium is justified once per scaffold, not per task.
...
<!--
This agent MIRRORS infra-architect.md verbatim except for the model and
description. Greenfield-scaffold is a one-shot architectural decision that
shapes every later phase — running it on Opus 4.7 instead of Sonnet pays
for itself by avoiding bad-stack-pick rework that compounds across tasks.

If you change the operational content of infra-architect.md, MIRROR THE
SAME CHANGE HERE. There is no skill-level model override in Claude Code,
so a dedicated agent file is the only way to scope the model upgrade to
greenfield-scaffold without affecting the other 7 skills that share
infra-architect (run-init, increase-testability, generate-architecture,
add-posthog, create-plan, extract-business-rules, quick-scan).
-->
